...
const ArticleWidget = ({slug}: {slug?: any}) => {
    ...
    return (
        <div className={`p-6 mb-8 rounded-lg border-2 border-y-slate-300 border-x-pink-500 dark:border-y-slate-500`}>
            <h3 className={`text-medium sm:text-2xl font-bold w-full text-slate-800 dark:text-slate-300 pb-4`}>{slug ? 'Related Posts': 'Recent Posts'}</h3>
            {relatedPosts.map((article, index) => (
                <Link key={index} href={`/blogs/${article.slug}`} className={`${inter.className}`}>
                    <div className='flex items-start my-4'>
                        <div className={`text-pink-500 font-bold text-sm text-left transform hover:scale-[1.1] transition-all mr-2`}>{`0${index + 1}`}</div>
